...
class ArticleHandler extends Handler {
	...
	function view($args, &$request) {
		...
		//$templateMgr->addJavaScript('js/pdf_viewer/articleView.js');
		//$templateMgr->addJavaScript('js/pdf_viewer/pdfobject.js');
  ...
  "rows": [
	...
	function download($args, &$request) {		
		...
		if ($article && $galley && !HookRegistry::call('ArticleHandler::downloadFile', array(&$article, &$galley))) {
			...
			if($galleyId == 'pdf')
			{
				$articleFileManager->downloadFile($galley->getFileId(),null,false,$article->getBestArticleId($journal).'.pdf');
			}
			else{
	...
	function downloadSuppFile($args, &$request) {
		...
		if ($article && $suppFile) {
			...
			if ($suppFile->isInlineable()) {
				...
				$articleFileManager->downloadFile($suppFile->getFileId(),null,false,$suppFileName);
			}
		}
	}
