...
class SubmissionReviewHandler extends ReviewerHandler {
	...
	function saveReviewFormResponse($args, $request) {
		...
		/*
		 * RDLG
		 * We will try to recieve an extra argument, to use it as an anchor for the redirect page
		 */
		$reviewFormFlag = (string) array_shift($args);
		if($reviewFormFlag == '')
		{
			$reviewFormFlag = null;
		}
		...
		if (ReviewerAction::saveReviewFormResponse($reviewId, $reviewFormId)) {
			...
			/*
			 * RDLG
			 * If reviewForm es diferent de blank, send email to editor
			 */
			 ...
			 * $journalName
			 *
			 * http://localhost/librelloph/index.php/politicsandgovernance/editor/viewReviewFormResponse/54/570
			 */
			$params = array(
				'editorialContactName' => $journal->getSetting('contactName'),
				'authorname' => $article->getAuthorString(),
				'articleID' => $article->getArticleId(),
				'articleTitle' => $article->getLocalizedTitle(),
				'journalName' => $journal->getJournalInitials(),
				'revName'=>$user->getFirstName() . ' '.$user->getLastName(),
				'revUserName'=>$user->getUsername(),
				'revID'=>$user->getUserId(),
				'urlReview'=>$journal->getUrl().'/editor/viewReviewFormResponse/'.$article->getArticleId().'/'.$reviewId
			);

			$emailKey = 'REVIEW_FORM_COMPLETE';
			...
			$mail->setSubject('article_id '.$article->getArticleId().', '.$mailSubject);
