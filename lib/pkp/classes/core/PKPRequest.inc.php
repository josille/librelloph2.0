...
/**
	 * Get the base URL of the submision (excluding script).
	 * @return string
	 */
		function getSubmissionBaseUrl() {
		$_this =& PKPRequest::_checkThis();

		if (!isset($_this->_submissionBaseUrl)) {
			$serverHost = $_this->getServerHost(null);
			//if ($serverHost !== null) {
				// Auto-detection worked.
			//	$_this->_baseUrl = $_this->getProtocol() . '://' . $_this->getServerHost() . $_this->getBasePath();
			//} else {
				// Auto-detection didn't work (e.g. this is a command-line call); use configuration param
				$_this->_submissionBaseUrl = Config::getVar('general', 'submission_base_url');
			//}
			HookRegistry::call('Request::getSubmissionBaseUrl', array(&$_this->_submissionBaseUrl));
		}

		return $_this->_submissionBaseUrl;
	}
