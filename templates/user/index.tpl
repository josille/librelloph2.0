	...
	<table width="100%" class="info">
		...
		{if $isValid.Author.$journalId}
			...
			<tr>
				...
				<td align="right">
                <a href="{url journal=$journalPath page="author" op="submit"}" style="background:none;">
                <!--{translate key="author.submit"}-->
                <img src="/public/site/newsubmission.png" alt="NewSubmissions" />
                </a>
                </td>
			</tr>
		{/if}
...
<ul class="plain">
	...
	{if $currentJournal}
		{if $journalPaymentsEnabled && $membershipEnabled}
			...
			{/if}
            <li>&#187; <a href="{url page="user" op="payMembershipInst"}">{translate key="payment.membership.buyMembershipInst"}</a></li>
		{/if}{* $journalPaymentsEnabled && $membershipEnabled *}
	{/if}{* $userJournal *}
