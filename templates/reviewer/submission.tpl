	...
	<td>
		...
		{if not $confirmedStatus}
			...
			{if !$submission->getCancelled()}
				{translate key="reviewer.article.canDoReview"} {icon name="envelope" width="49" height="30" url=$acceptUrl}
				&nbsp;&nbsp;&nbsp;&nbsp;
				{translate key="reviewer.article.cannotDoReview"} {icon name="envelope" width="49" height="30"  url=$declineUrl}
			{else}
...
{if $reviewAssignment->getReviewFormId()}
	...
	<tr valign="top">
		...
		jQuery.fn.reverse = function() {
			...
			function openWin()
			{
				var url = $('#rev_form_button').attr('href');
				$('#logo_loading').css('display','inline');		
				$('#result').load(url+' #reviewFormResponse',function(){
				
				$('#result #reviewFormResponse p input[value=Close]').css('display','none');

				/*
				Change url to add anchor
				*/
				var old_url = $('#result #reviewFormResponse form[name="saveReviewFormResponse"]').attr('action');
				
				$('#result #reviewFormResponse form[name="saveReviewFormResponse"]').removeAttr('action');
				$('#result #reviewFormResponse form[name="saveReviewFormResponse"]').removeAttr('method');

				$('#result #reviewFormResponse form[name="saveReviewFormResponse"]').submit(function() {
					
					$('.required_span').remove();
					...
					Check combobox
					*/
					$('#result #reviewFormResponse form[name="saveReviewFormResponse"] input:radio').each(function(index,value){
					  var name = $(this).attr("name");
					  
					  if($("input:radio[name="+name+"]:checked").length == 0)
					  {
						  input_text = $("input:radio[name="+name+"]").parent().prev().text();
						  arrFlags.push(input_text);
						  all_answered = false;
					  }
					});
					...
					Check text areas
					*/
					$('#result #reviewFormResponse form[name="saveReviewFormResponse"] textarea').each(function(index,value){
					
						if($(this).val().length ==0)
						{
							input_text = $(this).parent().prev().text();
							arrFlags.push(input_text);
						}
					});

					if($('#result #reviewFormResponse form[name="saveReviewFormResponse"] select').val().length == 0)
					{
						arrFlags.push('Please choose a "Recomendation"');
					}
					...
					for(var i=0; i<arrFlags.length;i++){
						...
						if(OldName != name)
						{
							$('#result #reviewFormResponse form[name="saveReviewFormResponse"] input[name=save]').parent().prepend("<div class='required_span' style='color:red;'>required "+name+" </div>");
						}
						OldName = name;
					}
					...
					/*
					Post of the form usign ajax, avoiding redirect
					*/
			       $.post(
			       		  old_url,
			              $(this).serialize(),
			              function(data){
			       			  /*
				              Activate buttons to let the form be subbmited
				              */
			       				$('#form_exist_alert').show()
			       				
			       				$('#submitReviewRecomendationInput').removeAttr('disabled');
			       				$('#recommendation_ddl').removeAttr('disabled');
			       				$('#submitReviewRecomendationInput').removeClass("submitReviewRecomendationdisabled").addClass("submitReviewRecomendation");
				...
				$( this ).dialog({
					title: "Review Form",
						 height: 600,
						 width: 500,
						 position: { my: "center top", at: "center bottom", of: logo_loading },
						 modal: true,
						 resizable: true,
			...
			{translate key="submission.reviewForm"} 
			{if $confirmedStatus and not $declined}
				<a class="icon" style='cursor:pointer;' onclick="openWin()">{icon name="comment"}</a>
				<a id='rev_form_button' style='visibility:hidden;' href="{url op="editReviewFormResponse" path=$reviewId|to_array:$reviewAssignment->getReviewFormId()}" target='blank'>TESTTT</a>
							
			<span id='form_exist_alert' style='color:red;display:{if $reviewFormResponseExists}inline{else}none{/if}'>Thank you for your review, please proced to the next step.</span>
			
				<img id='logo_loading' style='display:none;' src='{$baseUrl}/lib/pkp/templates/images/throbber.gif'>
				<div id="result"></div>
		...
		<table class="data" width="100%">
			<tr valign="top">
				...
				{else}
					...
					<input type="hidden" name="reviewId" value="{$reviewId|escape}" />
					<select id="recommendation_ddl"  name="recommendation" {if not $confirmedStatus or $declined or $submission->getCancelled() or (!$reviewFormResponseExists and !$reviewAssignment->getMostRecentPeerReviewComment() and !$uploadedFileExists)}disabled="disabled"{/if} class="selectMenu">
						{html_options_translate options=$reviewerRecommendationOptions selected=''}
					</select>&nbsp;&nbsp;&nbsp;&nbsp;
					<input id='submitReviewRecomendationInput' type="submit" name="submit" onclick="{literal}if($('#recommendation_ddl option:selected').val() == ''){alert('Please make a recommendation'); return false;}else{return confirmSubmissionCheck()}{/literal}" value="{translate key="reviewer.article.submitReview"}" {if not $confirmedStatus or $declined or $submission->getCancelled() or (!$reviewFormResponseExists and !$reviewAssignment->getMostRecentPeerReviewComment() and !$uploadedFileExists)}disabled="disabled"  class="submitReviewRecomendationdisabled" {else} class="submitReviewRecomendation" {/if} />
					<!-- input type="submit" name="submit" onclick="return confirmSubmissionCheck()" value="{translate key="reviewer.article.submitReview"}" {if not $confirmedStatus or $declined or $submission->getCancelled() or (!$reviewFormResponseExists and !$reviewAssignment->getMostRecentPeerReviewComment() and !$uploadedFileExists)}disabled="disabled"  class="submitReviewRecomendationdisabled" {else} class="submitReviewRecomendation" {/if} /-->
					</form>					
				{/if}
				</td>		
			</tr>
