...
class PubMedExportPlugin extends ImportExportPlugin {
	...
	function exportArticles(&$results, $outputFile = null) {
		...
		if (!empty($outputFile)) {
			...
			header("Content-Disposition: attachment; filename=\"".$article->getPublicArticleId().".xml\"");
	...
	function exportIssues(&$journal, &$issues, $outputFile = null) {
		...
		if (!empty($outputFile)) {
			...
			$filename_xml = String::strtolower($journal->getLocalizedSetting('initials')).'_vol'.$issue->getVolume().'_issue'.$issue->getNumber();;
						
			header("Content-Type: application/xml");
			header("Cache-Control: private");
			header("Content-Disposition: attachment; filename=\"".$filename_xml.".xml\"");
			XMLCustomWriter::printXML($doc);
		}
		return true;
	}
