...
class PubMedExportDom {
	...
	function &generateArticleDom(&$doc, &$journal, &$issue, &$section, &$article) {
		...
		/*
		 * ISSN TAG IS REQUIRED, can be empty
		 */
...
//		XMLCustomWriter::createChildWithText($doc, $root, 'Replaces', '');
		...
		foreach ($article->getAuthors() as $author) {
			...
			$authorNode =& PubMedExportDom::generateAuthorImprovedDom($doc, $author);
	...
	function &generateAuthorImprovedDom(&$doc, &$author) {
		$root =& XMLCustomWriter::createElement($doc, 'Author');
		...
		if ($author->getPrimaryContact()) {
			...
			if (is_array($affiliations)) foreach ($affiliations as $locale => $affiliation) {
				...
				XMLCustomWriter::createChildWithText($doc, $root, 'Affiliation', $affiliation . '. ' . $author->getEmail(), false);
			}
		}
		...
		if (is_array($affiliations)) foreach ($affiliations as $locale => $affiliation) {
			...
			foreach ($arr_affi as $index_affi => $name) {
				XMLCustomWriter::createChildWithText($doc, $root, 'Affiliation', $name . '. ' . $author->getEmail(), false);
			}
		}*/
