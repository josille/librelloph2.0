...
class PublishedArticle extends Article {
	...
	function getDOI($preview = false) {
		...
		// vol 9 issue 1 pag 4 - 14
		//johs2013.09010004
		//%j%y.%v%i%p
		
		//for special issues
		// 10.12924/cis2013.siIIPPPP
		...
		switch ( $doiSuffixSetting ) {
			...
			case 'pattern':
					...
					if($issue->getVolume() == 0 || $issue->getVolume() == '0' || $issue->getVolume() == '')
					{
					// %v - volume number
						$suffixPattern = String::regexp_replace('/%v/', 'si', $suffixPattern);
					}
					else {
						//$suffixPattern = String::regexp_replace('/%v/', $issue->getVolume(), $suffixPattern);
						$suffixPattern = String::regexp_replace('/%v/', sprintf('%1$02d', $issue->getVolume()), $suffixPattern);
					}
					// %i - issue number
					//$suffixPattern = String::regexp_replace('/%i/', $issue->getNumber(), $suffixPattern);
					$suffixPattern = String::regexp_replace('/%i/', sprintf('%1$02d', $issue->getNumber()), $suffixPattern);
					// %a - article id
					...
					// %f - first page number
					$pos = strpos($this->getPages(), "-");
					if(!$pos)
					{						
						$suffixPattern = String::regexp_replace('/%f/', sprintf('%1$04d', $this->getPages()), $suffixPattern);
					}
					else
					{
						$suffixPattern = String::regexp_replace('/%f/', sprintf('%1$04d', substr($this->getPages(), 0, $pos)), $suffixPattern);
					}
					// %y - year
