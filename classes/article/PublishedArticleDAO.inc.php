...
/**
	...
	function &getPublishedArticlesByJournalId($journalId = null, $rangeInfo = null, $reverse = false) {
		...
		$result =& $this->retrieveRange(
			...
			WHERE 	i.published = 1
				...
				AND a.status <> ' . STATUS_ARCHIVED . '
			ORDER BY date_published '. ($reverse?'DESC':'ASC').',pa.seq DESC ',
			$params,
			$rangeInfo
		);
	 ...
	 * @param $orderbySec boolean optional By RDLG
	 * @return PublishedArticle objects array
	 */
	function &getPublishedArticlesInSections($issueId, $useCache = false, $orderbySec = true) {
			//$firephp = FirePHP::getInstance(true);
		
			//$firephp->log($orderbySec, 'orderbySec');
			//$firephp->log($useCache, 'useCache');
		...
		/*
		 * br RDLG, ordeder by section
		 */
		$orderbySec_text = '';
		if($orderbySec)
		{
			$orderbySec_text = ' section_seq ASC, ';
		}
		...
		$result =& $this->retrieve(
			...
			WHERE	pa.article_id = a.article_id
				AND pa.issue_id = ?
				AND a.status <> ' . STATUS_ARCHIVED . '
			ORDER BY ' . $orderbySec_text . ' pa.seq ASC',
			array(
				$issueId,
				'title',
				$primaryLocale,
		...
		while (!$result->EOF) {
			...
			else 
			{
				...
				{				
					...
					//if (!$row['section_hide_title']) {
					//	$publishedArticles[0]['title'] = $publishedArticle->getSectionTitle();
					//}
				}
