...
{if $intro}<span itemprop="description">{$intro|nl2br}</span>{/if}
...
{iterate from=articles item=article}
	...
	{assign var=journalInitials value=$article->getJournalInitials()}
...
<div class="row row_over" id='row_over_{$article->getId()}'>
  ...
  <img src="/public/journals/{$article->getJournalId()}/{$journalInitials}_small_notext.png" style='float: left; 
  ...
  <div class="row">
   <div class="col-10 col-sm-10 col-lg-12">  
	<strong>
	<a style='vertical-align:super;' href="{url journal="$journalPath" page="article" op="view" path=$articlePath}">{$article->getArticleTitle()|strip_unsafe_html}</a>
			</strong>
			
	</div>
	
  <div class="col-10 col-sm-10 col-lg-12 font_size_10 margin_bottom_5">
  <strong>{$article->getDOI()}</strong>
  </div>
	</div></div>
...
<ul class="dropdown-menu" role="menu">
  ...
  <div class="col-10 col-sm-10 col-lg-12 font_size_12 margin_top_10">
	Publication Date: <strong>{$article->getDatePublished()|date_format:"%e %B %Y"}</strong>
	</div>
	</div>
...
<a class="margin_top_10 btn btn-sm btn-success accordion-toggle"  data-toggle="collapse"
	data-parent="#accordion_{$article->getId()}"
	href="#collapse_{$article->getId()}">
	<span class="glyphicon glyphicon-tags"></span> View abstract</a>
...
{if $article->getLocalizedAbstract()}
			<div id='abs_{$article->getId()}' class='article_abs_view'><strong>Abstract: </strong>{$article->getLocalizedAbstract()|strip_unsafe_html|nl2br}</div>
		{/if}
