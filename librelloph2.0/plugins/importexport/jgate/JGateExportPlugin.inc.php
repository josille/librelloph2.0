...
/**
 * @file plugins/importexport/crossref/JGateExportPlugin.inc.php
 *
 * Copyright (c) 2013 Rodrigo De la Garza
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class JGateExportPlugin
 * @ingroup plugins_importexport_jgate
 *
 * @brief JGATE XML metadata export plugin
 */
...
class JGateExportPlugin extends ImportExportPlugin {
	/**
	 ...
	 */
	function getName() {
		return 'JGateExportPlugin';
	}

	function getDisplayName() {
		return __('plugins.importexport.jgate.displayName');
	}

	function getDescription() {
		return __('plugins.importexport.jgate.description');
	}
	...
	function exportArticles(&$journal, &$results, $outputFile = null) {
		$this->import('JGateExportDom');

		$doc =& JGateExportDom::generateJGateDom();
		$articlesNode =& JGateExportDom::generateDoiBatchDom($doc);

		// Create Head Node and all parts inside it
		//$head =& JGateExportDom::generateHeadDom($doc, $journal);
		...
		//XMLCustomWriter::appendChild($doiBatchNode, $articleNode);
		...
		foreach ($results as $result) {
			...
			//$journalMetadataNode =& JGateExportDom::generateJournalMetadataDom($doc, $journal);
			//XMLCustomWriter::appendChild($journalNode, $journalMetadataNode);

			// Create the journal_issue node
			//$journalIssueNode =& JGateExportDom::generateJournalIssueDom($doc, $journal, $issue, $section, $article);
			//XMLCustomWriter::appendChild($journalNode, $journalIssueNode);

			// Create the article
			$ArticleNode =& JGateExportDom::generateArticleDom($doc, $journal, $issue, $section, $article);
			XMLCustomWriter::appendChild($articlesNode, $ArticleNode);

			// Create the DOI data
			//$DOIdataNode =& JGateExportDom::generateDOIdataDom($doc, $article->getDOI(), Request::url(null, 'article', 'view', $article->getId()));
	...
	function exportIssues(&$journal, &$issues, $outputFile = null) {
		...
		$doc =& JGateExportDom::generateJGateDom();
		$doiBatchNode =& JGateExportDom::generateDoiBatchDom($doc);
		...
		foreach ($issues as $issue) {
			foreach ($sectionDao->getSectionsForIssue($issue->getId()) as $section) {
				foreach ($publishedArticleDao->getPublishedArticlesBySectionId($section->getId(), $issue->getId()) as $article) {
					...
					// Article node
					$journalArticleNode =& JGateExportDom::generateJournalArticleDom($doc, $journal, $issue, $section, $article);
...
//		$command = array_shift($args);
		...
		if (!$journal) {
			if ($journalPath != '') {
				echo __('plugins.importexport.jgate.cliError') . "\n";
				echo __('plugins.importexport.jgate.error.unknownJournal', array('journalPath' => $journalPath)) . "\n\n";
			}
		...
		if ($xmlFile != '') switch (array_shift($args)) {
			case 'articles':
				...
				if (!$this->exportArticles($journal, $results, $xmlFile)) {
					echo __('plugins.importexport.jgate.cliError') . "\n";
					echo __('plugins.importexport.jgate.export.error.couldNotWrite', array('fileName' => $xmlFile)) . "\n\n";
				}
				return;
			case 'issue':
				$issueId = array_shift($args);
				...
				if ($issue == null) {
					echo __('plugins.importexport.jgate.cliError') . "\n";
					echo __('plugins.importexport.jgate.export.error.issueNotFound', array('issueId' => $issueId)) . "\n\n";
					return;
				}
				$issues = array($issue);
	...
	function usage($scriptName) {
		echo __('plugins.importexport.jgate.cliUsage', array(
